...
#titleNews 为 unicode
titleNews = parts.map(lambda p:p[1])
titleNewsHandled = titleNews.map(lambda line:handleAndCut(line))
temp = titleNewsHandled.map(lambda line:Row(title=line))
dfTitleNews = sqlContext.createDataFrame(temp)
dfTitleNews.show()
print(dfTitleNews.dtypes)
#dfTitleNews.write.text('roll_news_title_sina_com_cn.csv')
#dfTitleNews.write.text('test.txt')
sc.stop()
